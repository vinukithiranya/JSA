...
export default function TemplateBuilderPage({ user: _user, onLogout: _onLogout }: Props) {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-gray-50" onClick={() => { setShowCatMenu(false); }}>
      ...
      {/* ── Top Bar ── */}
      <div className="flex shrink-0 items-center gap-2 border-b border-brand-100 bg-white px-4 py-2.5 shadow-sm">
        <button onClick={() => navigate("/templates")}
          className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-sm font-medium text-brand-500 transition hover:bg-brand-50 hover:text-brand-700">
        ...
        <button onClick={undo} disabled={!history.length}
          className="rounded-lg p-2 text-brand-400 transition hover:bg-brand-50 hover:text-brand-700 disabled:opacity-30" title="Undo">
        ...
        <button onClick={redo} disabled={!future.length}
          className="rounded-lg p-2 text-brand-400 transition hover:bg-brand-50 hover:text-brand-700 disabled:opacity-30" title="Redo">
        ...
        {/* Save status */}
        <span className={`text-xs font-medium ${saveStatus === "saved" ? "text-green-600" : saveStatus === "saving" ? "text-brand-400" : "text-amber-600"}`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left Sidebar ── */}
        {tab === "build" && (
          <div className="flex w-48 shrink-0 flex-col overflow-hidden border-r border-brand-100 bg-white">
        ...
        {/* ── Center Canvas ── */}
        <div className="flex-1 overflow-y-auto p-6" onClick={() => setGlobalPanel(null)}>
          {tab === "build" ? (
