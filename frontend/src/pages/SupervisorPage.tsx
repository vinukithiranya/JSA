...
type ApprovedInfo = { id: string; job_number: string; boat_name: string };

export default function SupervisorPage({ user, onLogout }: Props) {
  const navigate                    = useNavigate();
  const [items, setItems]           = useState<JsaRecord[]>([]);
  const [signingJsa, setSigningJsa] = useState<JsaRecord | null>(null);
  ...
  function openSignModal(jsa: JsaRecord) {
    setSigningJsa(jsa);
    setIsEmpty(true);
  ...
  async function confirmApprove() {
    if (!signingJsa || isEmpty) return;
    setApproving(true);
    try {
      const canvas = canvasRef.current;
      if (!canvas) return;
      const signature = canvas.toDataURL("image/png");
      await api(`/api/jsa/${signingJsa.id}/approve`, {
        method: "POST",
        body: JSON.stringify({
          signature,
      ...
      setApproved({
        id:          signingJsa.id,
        job_number:  signingJsa.job_number,
        boat_name:   signingJsa.boat_name,
      });
      setSigningJsa(null);
      await load();
    } finally {
    ...
    <Layout user={user} title="Approval Queue" onLogout={onLogout}>
      ...
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-brand-600">
          ...
          {" "}JSA{items.length !== 1 ? "s" : ""} pending approval
        </p>
        <button
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-sm font-bold text-brand-900">{jsa.job_number}</h3>
                    <span className="rounded bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">
                      Pending
                    </span>
                  </div>
                  <p className="mt-0.5 text-sm text-brand-700">{jsa.boat_name}</p>
                  <div className="mt-1.5 flex flex-wrap gap-3 text-xs text-brand-500">
                    <span>SL: {jsa.service_log_number}</span>
                    <span>📍 {jsa.location}</span>
                    <span>📅 {String(jsa.date)}</span>
                    <span>
                      {jsa.hazards.length} hazard{jsa.hazards.length !== 1 ? "s" : ""}
                    </span>
                    <span>{jsa.steps.length} steps</span>
                  </div>
                </div>

                <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                  <button
                    onClick={() => navigate(`/jsa/report/${jsa.id}`)}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SIGNATURE MODAL — tablet-optimised full-screen overlay
      ════════════════════════════════════════════════════════════════════ */}
      {signingJsa && (
          ...
          >
            <div>
              ...
              <p className="text-xs text-green-100">
                {signingJsa.job_number} — {signingJsa.boat_name}
              </p>
            </div>
            <button
              onClick={() => setSigningJsa(null)}
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            <div className="rounded-xl border border-brand-200 bg-brand-50 px-4 py-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="ml-auto text-right text-xs text-brand-500">
                  ...
                  <p>This name appears on the PDF</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-brand-100 bg-white px-4 py-3 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-brand-700 sm:grid-cols-3">
                <div><span className="font-semibold">Job:</span> {signingJsa.job_number}</div>
                <div><span className="font-semibold">Vessel:</span> {signingJsa.boat_name}</div>
                <div><span className="font-semibold">Location:</span> {signingJsa.location}</div>
                <div><span className="font-semibold">Date:</span> {String(signingJsa.date)}</div>
                <div><span className="font-semibold">Steps:</span> {signingJsa.steps.length}</div>
                <div><span className="font-semibold">Hazards:</span> {signingJsa.hazards.length}</div>
              </div>
            </div>
            ...
            <div>
              ...
              {/* Canvas container — white background, thick border */}
              <div
              ...
              <p className="mt-2 text-center text-xs text-brand-400">
                Your signature will be embedded in the PDF document
              </p>
            </div>

            {/* Legal acknowledgement */}
            <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-xs text-amber-800">
              By signing above you confirm that you have reviewed this Job Safety Assessment
              and approve it for execution. This approval is legally binding under the applicable
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SUCCESS MODAL — shown after approval with PDF download
      ════════════════════════════════════════════════════════════════════ */}
      {approved && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            >
              ...
              <h2 className="text-xl font-bold text-white">JSA Approved!</h2>
              <p className="mt-1 text-sm text-green-100">
                {approved.job_number} — {approved.boat_name}
              </p>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="rounded-lg bg-brand-50 border border-brand-200 px-4 py-3 text-xs text-brand-700 space-y-1">
                <p>
                  ...
                  <span className="font-bold text-brand-900">{user?.full_name}</span>
                </p>
                <p>
                ...
                <p className="text-brand-500">
                  Signature and approver details are now embedded in the PDF.
                </p>
              </div>

              {/* Primary CTA — open the signed report */}
              <button
                onClick={() => { setApproved(null); navigate(`/jsa/report/${approved.id}`); }}
